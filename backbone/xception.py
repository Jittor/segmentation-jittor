...
class SeparableConv2d(Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, dilation=1, bias=False):
        ...
        self.conv1 = nn.Conv(inplanes, inplanes, kernel_size, stride, 0, dilation,
                               groups=inplanes, bias=bias)

        self.bn = nn.BatchNorm(inplanes)
        self.pointwise = nn.Conv(inplanes, planes, 1, 1, 0, 1, 1, bias=bias)
...
class Block(Module):
    def __init__(self, inplanes, planes, reps, stride=1, dilation=1,
        ...
        if planes != inplanes or stride != 1:
            self.skip = nn.Conv(inplanes, planes, 1, stride=stride, bias=False)
            self.skipbn = nn.BatchNorm(planes)
        else:
            self.skip = None
        ...
        if grow_first:
            ...
            rep.append(nn.BatchNorm(planes))
            filters = planes

        for i in range(reps - 1):
            rep.append(self.relu)
            rep.append(SeparableConv2d(filters, filters, 3, 1, dilation))
            rep.append(nn.BatchNorm(filters))
...
class AlignedXception(Module):
    ...
    def __init__(self, output_stride, pretrained=True):
        ...
        # Entry flow
        self.conv1 = nn.Conv(3, 32, 3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm(32)
        self.relu = nn.ReLU()

        self.conv2 = nn.Conv(32, 64, 3, stride=1, padding=1, bias=False)
